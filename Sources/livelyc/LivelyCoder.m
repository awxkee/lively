...
//  Lively
//
//  Created by Radzivon Bartoshyk on 15/5/13.
//  Copyright (c) 2022 Radzivon Bartoshyk.
//
//  This source code is licensed under the MIT-style license found in the
//  LICENSE file in the root directory of this source tree.
//

#import "LivelyCoder.h"
#import "Lively.h"
...
#define LV_FOUR_CC(c1,c2,c3,c4) ((uint32_t)(((c4) << 24) | ((c3) << 16) | ((c2) << 8) | (c1)))
#define LV_TWO_CC(c1,c2) ((uint16_t)(((c2) << 8) | (c1)))

static inline uint16_t lv_swap_endian_uint16(uint16_t value) {
    return
    (uint16_t) ((value & 0x00FF) << 8) |
    (uint16_t) ((value & 0xFF00) >> 8) ;
}

static inline uint32_t lv_swap_endian_uint32(uint32_t value) {
    return
...
typedef enum {
    LV_PNG_ALPHA_TYPE_PALEETE = 1 << 0,
    LV_PNG_ALPHA_TYPE_COLOR = 1 << 1,
    LV_PNG_ALPHA_TYPE_ALPHA = 1 << 2,
} lv_png_alpha_type;

typedef enum {
    LV_PNG_DISPOSE_OP_NONE = 0,
    LV_PNG_DISPOSE_OP_BACKGROUND = 1,
    LV_PNG_DISPOSE_OP_PREVIOUS = 2,
} lv_png_dispose_op;

typedef enum {
    LV_PNG_BLEND_OP_SOURCE = 0,
    LV_PNG_BLEND_OP_OVER = 1,
} lv_png_blend_op;
...
typedef struct {
    ...
    uint8_t color_type;         ///< see lv_png_alpha_type
...
typedef struct {
    ...
    uint8_t dispose_op;        ///< see lv_png_dispose_op
    uint8_t blend_op;          ///< see lv_png_blend_op
} lv_png_chunk_fcTL;
...
typedef struct {
    ...
    lv_png_chunk_fcTL frame_control;
} lv_png_frame_info;

typedef struct {
    lv_png_chunk_IHDR header;   ///< png header
    lv_png_chunk_info *chunks;      ///< chunks
    uint32_t chunk_num;          ///< count of chunks
    
    lv_png_frame_info *apng_frames; ///< frame info, NULL if not apng
...
static void lv_png_chunk_IHDR_read(lv_png_chunk_IHDR *IHDR, const uint8_t *data) {
    IHDR->width = lv_swap_endian_uint32(*((uint32_t *)(data)));
    IHDR->height = lv_swap_endian_uint32(*((uint32_t *)(data + 4)));
...
static void lv_png_chunk_IHDR_write(lv_png_chunk_IHDR *IHDR, uint8_t *data) {
    *((uint32_t *)(data)) = lv_swap_endian_uint32(IHDR->width);
    *((uint32_t *)(data + 4)) = lv_swap_endian_uint32(IHDR->height);
...
static void lv_png_chunk_fcTL_read(lv_png_chunk_fcTL *fcTL, const uint8_t *data) {
    fcTL->sequence_number = lv_swap_endian_uint32(*((uint32_t *)(data)));
    fcTL->width = lv_swap_endian_uint32(*((uint32_t *)(data + 4)));
    fcTL->height = lv_swap_endian_uint32(*((uint32_t *)(data + 8)));
    fcTL->x_offset = lv_swap_endian_uint32(*((uint32_t *)(data + 12)));
    fcTL->y_offset = lv_swap_endian_uint32(*((uint32_t *)(data + 16)));
    fcTL->delay_num = lv_swap_endian_uint16(*((uint16_t *)(data + 20)));
    fcTL->delay_den = lv_swap_endian_uint16(*((uint16_t *)(data + 22)));
    fcTL->dispose_op = data[24];
    fcTL->blend_op = data[25];
}

static void lv_png_chunk_fcTL_write(lv_png_chunk_fcTL *fcTL, uint8_t *data) {
    *((uint32_t *)(data)) = lv_swap_endian_uint32(fcTL->sequence_number);
    *((uint32_t *)(data + 4)) = lv_swap_endian_uint32(fcTL->width);
    *((uint32_t *)(data + 8)) = lv_swap_endian_uint32(fcTL->height);
    *((uint32_t *)(data + 12)) = lv_swap_endian_uint32(fcTL->x_offset);
    *((uint32_t *)(data + 16)) = lv_swap_endian_uint32(fcTL->y_offset);
    *((uint16_t *)(data + 20)) = lv_swap_endian_uint16(fcTL->delay_num);
    *((uint16_t *)(data + 22)) = lv_swap_endian_uint16(fcTL->delay_den);
    data[24] = fcTL->dispose_op;
    data[25] = fcTL->blend_op;
}

// convert double value to fraction
static void lv_png_delay_to_fraction(double duration, uint16_t *num, uint16_t *den) {
    if (duration >= 0xFF) {
        *num = 0xFF;
        *den = 1;
...
static double lv_png_delay_to_seconds(uint16_t num, uint16_t den) {
    if (den == 0) {
        return num / 100.0;
    } else {
...
static bool lv_png_validate_animation_chunk_order(lv_png_chunk_info *chunks,  /* input */
    ...
    if (chunks->fourcc != LV_FOUR_CC('I', 'H', 'D', 'R')) return false;
    if ((chunks + chunk_num - 1)->fourcc != LV_FOUR_CC('I', 'E', 'N', 'D')) return false;
    ...
    for (uint32_t i = 0; i < chunk_num; i++) {
        lv_png_chunk_info *chunk = chunks + i;
        switch (chunk->fourcc) {
            case LV_FOUR_CC('I', 'H', 'D', 'R'): {  // png header
                if (i != 0) return false;
                if (IHDR_num > 0) return false;
                IHDR_num++;
            } break;
            case LV_FOUR_CC('I', 'D', 'A', 'T'): {  // png data
                if (prev_fourcc != LV_FOUR_CC('I', 'D', 'A', 'T')) {
                    if (IDAT_num == 0)
                        first_IDAT = i;
                    else
                        return false;
            ...
            case LV_FOUR_CC('a', 'c', 'T', 'L'): {  // apng control
                if (acTL_num > 0) return false;
                acTL_num++;
            } break;
            case LV_FOUR_CC('f', 'c', 'T', 'L'): {  // apng frame control
                if (i + 1 == chunk_num) return false;
                if ((chunk + 1)->fourcc != LV_FOUR_CC('f', 'd', 'A', 'T') &&
                    (chunk + 1)->fourcc != LV_FOUR_CC('I', 'D', 'A', 'T')) {
                    return false;
                }
                if (fcTL_num == 0) {
                    if ((chunk + 1)->fourcc == LV_FOUR_CC('I', 'D', 'A', 'T')) {
                        first_frame_cover = true;
                    }
                }
                fcTL_num++;
            } break;
            case LV_FOUR_CC('f', 'd', 'A', 'T'): {  // apng data
                if (prev_fourcc != LV_FOUR_CC('f', 'd', 'A', 'T') && prev_fourcc != LV_FOUR_CC('f', 'c', 'T', 'L')) {
                    return false;
                }
            } break;
        }
...
static void lv_png_info_release(lv_png_info *info) {
    if (info) {
...
/**
 ...
 @return A png info object, you may call lv_png_info_release() to release it.
 Returns NULL if an error occurs.
 */
static lv_png_info *lv_png_info_create(const uint8_t *data, uint32_t length) {
    if (length < 32) return NULL;
    if (*((uint32_t *)data) != LV_FOUR_CC(0x89, 0x50, 0x4E, 0x47)) return NULL;
    if (*((uint32_t *)(data + 4)) != LV_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A)) return NULL;
    
    uint32_t chunk_realloc_num = 16;
    lv_png_chunk_info *chunks = malloc(sizeof(lv_png_chunk_info) * chunk_realloc_num);
    ...
    do {
        if (chunk_num >= chunk_capacity) {
            lv_png_chunk_info *new_chunks = realloc(chunks, sizeof(lv_png_chunk_info) * (chunk_capacity + chunk_realloc_num));
            if (!new_chunks) {
                free(chunks);
                return NULL;
            }
        ...
        lv_png_chunk_info *chunk = chunks + chunk_num;
        const uint8_t *chunk_data = data + offset;
        chunk->offset = offset;
        chunk->length = lv_swap_endian_uint32(*((uint32_t *)chunk_data));
        ...
        chunk->crc32 = lv_swap_endian_uint32(*((uint32_t *)(chunk_data + 8 + chunk->length)));
        chunk_num++;
        offset += 12 + chunk->length;
        
        switch (chunk->fourcc) {
            case LV_FOUR_CC('a', 'c', 'T', 'L') : {
                if (chunk->length == 8) {
                    apng_frame_number = lv_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
                    apng_loop_num = lv_swap_endian_uint32(*((uint32_t *)(chunk_data + 12)));
                } else {
                    apng_chunk_error = true;
                }
            } break;
            case LV_FOUR_CC('f', 'c', 'T', 'L') :
            case LV_FOUR_CC('f', 'd', 'A', 'T') : {
                if (chunk->fourcc == LV_FOUR_CC('f', 'c', 'T', 'L')) {
                    if (chunk->length != 26) {
                        apng_chunk_error = true;
                    } else {
                        apng_frame_index++;
                ...
                if (chunk->length > 4) {
                    uint32_t sequence = lv_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
    ...
    if (chunk_num < 3 ||
        chunks->fourcc != LV_FOUR_CC('I', 'H', 'D', 'R') ||
        chunks->length != 13) {
        free(chunks);
        return NULL;
    }
    ...
    // png info
    lv_png_info *info = calloc(1, sizeof(lv_png_info));
    if (!info) {
        free(chunks);
        return NULL;
    }
    ...
    lv_png_chunk_IHDR_read(&info->header, data + chunks->offset + 8);
    ...
    if (!apng_chunk_error && apng_frame_number == apng_frame_index && apng_frame_number >= 1) {
        ...
        if (!lv_png_validate_animation_chunk_order(info->chunks, info->chunk_num, &first_IDAT_index, &first_frame_is_cover)) {
        ...
        info->apng_frames = calloc(apng_frame_number, sizeof(lv_png_frame_info));
        if (!info->apng_frames) {
            lv_png_info_release(info);
            return NULL;
        }
        ...
        for (int32_t i = 0; i < info->chunk_num; i++) {
            lv_png_chunk_info *chunk = info->chunks + i;
            switch (chunk->fourcc) {
                case LV_FOUR_CC('I', 'D', 'A', 'T'): {
                    if (info->apng_shared_insert_index == 0) {
                        info->apng_shared_insert_index = i;
                    }
                    if (first_frame_is_cover) {
                        lv_png_frame_info *frame = info->apng_frames + frame_index;
                        frame->chunk_num++;
                ...
                case LV_FOUR_CC('f', 'c', 'T', 'L'): {
                    ...
                    lv_png_chunk_fcTL_read(&frame->frame_control, data + chunk->offset + 8);
                } break;
                case LV_FOUR_CC('f', 'd', 'A', 'T'): {
                    lv_png_frame_info *frame = info->apng_frames + frame_index;
                    frame->chunk_num++;
...
/**
 ...
 Returns NULL if an error occurs.
 */
static uint8_t *lv_png_copy_frame_data_at_index(const uint8_t *data,
                                                const lv_png_info *info,
                                                const uint32_t index,
                                                uint32_t *size) {
    if (index >= info->apng_frame_num) return NULL;
    
    lv_png_frame_info *frame_info = info->apng_frames + index;
    ...
    *size = frame_remux_size;
    ...
    for (uint32_t i = 0; i < info->apng_shared_chunk_num; i++) {
        ...
        lv_png_chunk_info *shared_chunk_info = info->chunks + shared_chunk_index;
        ...
        if (shared_chunk_index >= info->apng_shared_insert_index && !inserted) { // replace IDAT with fdAT
            ...
            for (uint32_t c = 0; c < frame_info->chunk_num; c++) {
                lv_png_chunk_info *insert_chunk_info = info->chunks + frame_info->chunk_index + c;
                if (insert_chunk_info->fourcc == LV_FOUR_CC('f', 'd', 'A', 'T')) {
                    *((uint32_t *)(frame_data + data_offset)) = lv_swap_endian_uint32(insert_chunk_info->length - 4);
                    *((uint32_t *)(frame_data + data_offset + 4)) = LV_FOUR_CC('I', 'D', 'A', 'T');
                    ...
                    *((uint32_t *)(frame_data + data_offset + insert_chunk_info->length + 4)) = lv_swap_endian_uint32(crc);
        ...
        if (shared_chunk_info->fourcc == LV_FOUR_CC('I', 'H', 'D', 'R')) {
            uint8_t tmp[25] = {0};
            memcpy(tmp, data + shared_chunk_info->offset, 25);
            lv_png_chunk_IHDR IHDR = info->header;
            IHDR.width = frame_info->frame_control.width;
            IHDR.height = frame_info->frame_control.height;
            lv_png_chunk_IHDR_write(&IHDR, tmp + 8);
            *((uint32_t *)(tmp + 21)) = lv_swap_endian_uint32((uint32_t)crc32(0, tmp + 4, 17));
...
static inline size_t LivelyByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

/// Convert degree to radians
static inline CGFloat LivelyDegreesToRadians(CGFloat degrees) {
    return degrees * M_PI / 180;
}

CGColorSpaceRef LVCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef LVCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL LVCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, LVCGColorSpaceGetDeviceRGB());
}

BOOL LVCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, LVCGColorSpaceGetDeviceGray());
}
...
/**
 ...
 CGDataProviderRef provider = CGDataProviderCreateWithData(data, data, size, LVCGDataProviderReleaseDataCallback);
 */
static void LVCGDataProviderReleaseDataCallback(void *info, const void *data, size_t size) {
    if (info) free(info);
}
 ...
 */
static BOOL LVCGImageDecodeToBitmapBufferWithAnyFormat(CGImageRef srcImage, vImage_Buffer *dest, vImage_CGImageFormat *destFormat) {
...
/**
 ...
 @return Whether succeed.
 */
static BOOL LVCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = LVCGColorSpaceGetDeviceRGB();
    destFormat.bitmapInfo = bitmapInfo;
    dest->data = NULL;
    if (LVCGImageDecodeToBitmapBufferWithAnyFormat(srcImage, dest, &destFormat)) return YES;
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, LVCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef LVCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, LVCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef LVCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    if(!LVCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &src, kCGImageAlphaFirst | kCGBitmapByteOrderDefault)) return NULL;
    
    size_t destBytesPerRow = LivelyByteAlign(destWidth * 4, 32);
    ...
    tmpProvider = CGDataProviderCreateWithData(tmp.data, tmp.data, destHeight * destBytesPerRow, LVCGDataProviderReleaseDataCallback);
    if (!tmpProvider) goto fail;
    tmp.data = NULL; // hold by provider
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, LVCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    if (!LVCGImageDecodeToBitmapBufferWith32BitFormat(tmpImage, &dest, destBitmapInfo)) goto fail;
    CFRelease(tmpImage);
    tmpImage = NULL;
    
    destProvider = CGDataProviderCreateWithData(dest.data, dest.data, destHeight * destBytesPerRow, LVCGDataProviderReleaseDataCallback);
    if (!destProvider) goto fail;
    dest.data = NULL; // hold by provider
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, LVCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation LVUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger LVUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef LVCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    switch (orientation) {
        case UIImageOrientationDown: {
            transform = CGAffineTransformMakeRotation(LivelyDegreesToRadians(180));
            transform = CGAffineTransformTranslate(transform, -(CGFloat)width, -(CGFloat)height);
        } break;
        case UIImageOrientationLeft: {
            transform = CGAffineTransformMakeRotation(LivelyDegreesToRadians(90));
    ...
    return LVCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

LivelyType LivelyDetectType(CFDataRef data) {
    if (!data) return LivelyTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return LivelyTypeUnknown;
    
    const char *bytes = (char *)CFDataGetBytePtr(data);
    
    uint32_t magic4 = *((uint32_t *)bytes);
    switch (magic4) {
        case LV_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { // big endian TIFF
            return LivelyTypeTIFF;
        } break;
            
        case LV_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { // little endian TIFF
            return LivelyTypeTIFF;
        } break;
            
        case LV_FOUR_CC(0x00, 0x00, 0x01, 0x00): { // ICO
            return LivelyTypeICO;
        } break;
            
        case LV_FOUR_CC(0x00, 0x00, 0x02, 0x00): { // CUR
            return LivelyTypeICO;
        } break;
            
        case LV_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return LivelyTypeICNS;
        } break;
            
        case LV_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return LivelyTypeGIF;
        } break;
            
        case LV_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == LV_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return LivelyTypePNG;
            }
        } break;
            
        case LV_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            uint32_t tmp = *((uint32_t *)(bytes + 8));
            if (tmp == LV_FOUR_CC('W', 'E', 'B', 'P')) {
                return LivelyTypeWebP;
            }
        } break;
            /*
             case LV_FOUR_CC('B', 'P', 'G', 0xFB): { // BPG
             return LivelyTypeBPG;
             } break;
             */
    }
    ...
    switch (magic2) {
        ...
        case LV_TWO_CC('P', 'I'):
        case LV_TWO_CC('C', 'I'):
        case LV_TWO_CC('C', 'P'): { // BMP
            return LivelyTypeBMP;
        }
        case LV_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return LivelyTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return LivelyTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return LivelyTypeJPEG2000;
    
    AVIFAnimatedDecoder* avifDecoder = [[AVIFAnimatedDecoder alloc] initWithData:(__bridge NSData * _Nonnull)(data)];
    if (avifDecoder) {
        return LivelyTypeAVIF;
    }
    
    return LivelyTypeUnknown;
}

CFStringRef LivelyTypeToUTType(LivelyType type) {
    switch (type) {
        case LivelyTypeJPEG: return kUTTypeJPEG;
        case LivelyTypeJPEG2000: return kUTTypeJPEG2000;
        case LivelyTypeTIFF: return kUTTypeTIFF;
        case LivelyTypeBMP: return kUTTypeBMP;
        case LivelyTypeICO: return kUTTypeICO;
        case LivelyTypeICNS: return kUTTypeAppleICNS;
        case LivelyTypeGIF: return kUTTypeGIF;
        case LivelyTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

LivelyType LivelyTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(LivelyTypeJPEG),
                (id)kUTTypeJPEG2000 : @(LivelyTypeJPEG2000),
                (id)kUTTypeTIFF : @(LivelyTypeTIFF),
                (id)kUTTypeBMP : @(LivelyTypeBMP),
                (id)kUTTypeICO : @(LivelyTypeICO),
                (id)kUTTypeAppleICNS : @(LivelyTypeICNS),
                (id)kUTTypeGIF : @(LivelyTypeGIF),
                (id)kUTTypePNG : @(LivelyTypePNG)};
    });
    if (!uti) return LivelyTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *LivelyTypeGetExtension(LivelyType type) {
    switch (type) {
        case LivelyTypeJPEG: return @"jpg";
        case LivelyTypeJPEG2000: return @"jp2";
        case LivelyTypeTIFF: return @"tiff";
        case LivelyTypeBMP: return @"bmp";
        case LivelyTypeICO: return @"ico";
        case LivelyTypeICNS: return @"icns";
        case LivelyTypeGIF: return @"gif";
        case LivelyTypePNG: return @"png";
        case LivelyTypeWebP: return @"webp";
        case LivelyTypeAVIF: return @"avif";
        default: return nil;
    }
}

CFDataRef LVCGImageCreateEncodedData(CGImageRef imageRef, LivelyType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == LivelyTypeWebP) {
        if (quality == 1) {
            return LVCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, LivelyPresetDefault);
        } else {
            return LVCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, LivelyPresetDefault);
        }
    }
    
    CFStringRef uti = LivelyTypeToUTType(type);
    if (!uti) return nil;
...
BOOL LivelyWebPAvailable() {
    return YES;
}

CFDataRef LVCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, LivelyPreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    if(!LVCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &buffer, kCGImageAlphaLast | kCGBitmapByteOrderDefault)) return nil;
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > LivelyPresetText ? LivelyPresetDefault : preset;
...
NSUInteger LivelyGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef LVCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    bitsPerComponent = 8;
    bitsPerPixel = 32;
    bytesPerRow = LivelyByteAlign(bitsPerPixel / 8 * canvasWidth, 32);
    ...
    provider = CGDataProviderCreateWithData(destBytes, destBytes, destLength, LVCGDataProviderReleaseDataCallback);
    if (!provider) goto fail;
    destBytes = NULL; // hold by provider
    
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, LVCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation LivelyFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    LivelyFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    LivelyFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _LivelyDecoderFrame : LivelyFrame
...
@implementation _LivelyDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _LivelyDecoderFrame *frame = [super copyWithZone:zone];
...
- (void)dealloc {
    ...
    if (_apngSource) lv_png_info_release(_apngSource);
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    LivelyDecoder *decoder = [[LivelyDecoder alloc] initWithScale:scale];
...
- (LivelyFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    LivelyFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_LivelyDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    LivelyType type = LivelyDetectType((__bridge CFDataRef)data);
...
- (LivelyFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _LivelyDecoderFrame *frame = [(_LivelyDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != LivelyTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = LVCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
        ...
        CFRelease(imageRef);
        if (!image) return nil;
        image.lv_isDecodedForDisplay = decoded;
        frame.image = image;
        return frame;
    }
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == LivelyDisposeBackground) {
    ...
    image.lv_isDecodedForDisplay = YES;
    frame.image = image;
    if (extendToCanvas) {
        frame.width = _width;
        frame.height = _height;
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = LivelyDisposeNone;
        frame.blend = LivelyBlendNone;
    }
    return frame;
}
...
- (void)_updateSourceAVIF {
    ...
    avifDecoder = [[AVIFAnimatedDecoder alloc] initWithData:_data];
    if (!avifDecoder) return;
    
    CGSize imageSize = [avifDecoder imageSize];
    uint32_t canvasWidth = imageSize.width;
    uint32_t canvasHeight = imageSize.height;
    ...
    int framesCount = [avifDecoder framesCount];
    for (int i = 0; i < framesCount; i++) {
        _LivelyDecoderFrame *frame = [_LivelyDecoderFrame new];
        [frames addObject:frame];
        
        int duration = [avifDecoder frameDuration:i];
        frame.index = i;
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            ...
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                frame.dispose = LivelyDisposeBackground;
            }
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = LivelyBlendOver;
            }
...
- (void)_updateSourceAPNG {
    ...
    lv_png_info_release(_apngSource);
    _apngSource = nil;
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        lv_png_frame_info *fi = apng->apng_frames + i;
        frame.index = i;
        frame.duration = lv_png_delay_to_seconds(fi->frame_control.delay_num, fi->frame_control.delay_den);
        frame.hasAlpha = YES;
        ...
        switch (fi->frame_control.dispose_op) {
            case LV_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = LivelyDisposeBackground;
            } break;
            case LV_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = LivelyDisposePrevious;
            } break;
            default: {
                frame.dispose = LivelyDisposeNone;
            } break;
        ...
        if (frame.blend == LivelyBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != LivelyDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == LivelyDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == LivelyTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == LivelyTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = LVUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _LivelyDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = LVCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, LVCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (avifDecoder) {
        CGImageRef imageRef = [avifDecoder get:(int)index];
        if (imageRef && extendToCanvas) {
            size_t width = CGImageGetWidth(imageRef);
            size_t height = CGImageGetHeight(imageRef);
            if (width == _width && height == _height) {
    ...
    if (_apngSource) {
        uint32_t size = 0;
        uint8_t *bytes = lv_png_copy_frame_data_at_index(_data.bytes, _apngSource, (uint32_t)index, &size);
        if (!bytes) return NULL;
        CGDataProviderRef provider = CGDataProviderCreateWithData(bytes, bytes, size, LVCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(bytes);
            return NULL;
        }
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, LVCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        size_t bytesPerRow = LivelyByteAlign(bitsPerPixel / 8 * width, 32);
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(pixels, pixels, length, LVCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(pixels);
            return NULL;
        }
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, LVCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, LVCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(_LivelyDecoderFrame *)frame {
    if (frame.dispose == LivelyDisposePrevious) {
        // nothing
    } else if (frame.dispose == LivelyDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == LivelyBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(_LivelyDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == LivelyDisposePrevious) {
        if (frame.blend == LivelyBlendOver) {
...
@implementation LivelyEncoder {
    NSMutableArray *_images;
    NSMutableArray *_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"LivelyEncoder init error" reason:@"LivelyEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
    return [self initWithType:LivelyTypeUnknown];
}

- (instancetype)initWithType:(LivelyType)type {
    if (type == LivelyTypeUnknown || type >= LivelyTypeOther) {
        NSLog(@"[%s: %d] Unsupported image type:%d",__FUNCTION__, __LINE__, (int)type);
        return nil;
    }
    
    if (type == LivelyTypeWebP) {
        NSLog(@"[%s: %d] WebP is not available, check the documentation to see how to install WebP component: https://github.com/Radzivon Bartoshyk/Lively#installation", __FUNCTION__, __LINE__);
        return nil;
    }
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, LivelyTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, LivelyTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == LivelyTypeGIF && count > 1) {
            ...
            if ([imageSrc isKindOfClass:[UIImage class]]) {
                ...
                if (image.imageOrientation != UIImageOrientationUp && image.CGImage) {
                    ...
                    CGImageRef rotated = LVCGImageCreateCopyWithOrientation(image.CGImage, image.imageOrientation, info);
                    if (rotated) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return LVCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return LVCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == LivelyTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = LVCGImageCreateEncodedData(decoded, LivelyTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, LVCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = LVCGImageCreateEncodedData(extendedImage, LivelyTypePNG, 1);
        if (!frameData) {
    ...
    lv_png_info *info = lv_png_info_create(firstFrameData.bytes, (uint32_t)firstFrameData.length);
    if (!info) return nil;
    ...
    png_header[0] = LV_FOUR_CC(0x89, 0x50, 0x4E, 0x47);
    png_header[1] = LV_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A);
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == LV_FOUR_CC('I', 'D', 'A', 'T')) {
            insertBefore = YES;
            // insert acTL (APNG Control)
            uint32_t acTL[5] = {0};
            acTL[0] = lv_swap_endian_uint32(8); //length
            acTL[1] = LV_FOUR_CC('a', 'c', 'T', 'L'); // fourcc
            acTL[2] = lv_swap_endian_uint32((uint32_t)pngDatas.count); // num frames
            acTL[3] = lv_swap_endian_uint32((uint32_t)_loopCount); // num plays
            acTL[4] = lv_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(acTL + 1), 12)); //crc32
            [result appendBytes:acTL length:20];
            
            // insert fcTL (first frame control)
            lv_png_chunk_fcTL chunk_fcTL = {0};
            ...
            lv_png_delay_to_fraction([(NSNumber *)_durations[0] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
            chunk_fcTL.delay_num = chunk_fcTL.delay_num;
            chunk_fcTL.delay_den = chunk_fcTL.delay_den;
            chunk_fcTL.dispose_op = LV_PNG_DISPOSE_OP_BACKGROUND;
            chunk_fcTL.blend_op = LV_PNG_BLEND_OP_SOURCE;
            
            uint8_t fcTL[38] = {0};
            *((uint32_t *)fcTL) = lv_swap_endian_uint32(26); //length
            *((uint32_t *)(fcTL + 4)) = LV_FOUR_CC('f', 'c', 'T', 'L'); // fourcc
            lv_png_chunk_fcTL_write(&chunk_fcTL, fcTL + 8);
            *((uint32_t *)(fcTL + 34)) = lv_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
            [result appendBytes:fcTL length:38];
            
            apngSequenceIndex++;
        }
        
        if (!insertAfter && insertBefore && chunk->fourcc != LV_FOUR_CC('I', 'D', 'A', 'T')) {
            insertAfter = YES;
            // insert fcTL and fdAT (APNG frame control and data)
            
            for (int i = 1; i < pngDatas.count; i++) {
                NSData *frameData = pngDatas[i];
                lv_png_info *frame = lv_png_info_create(frameData.bytes, (uint32_t)frameData.length);
                if (!frame) {
                    lv_png_info_release(info);
                    return nil;
                }
                ...
                lv_png_delay_to_fraction([(NSNumber *)_durations[i] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
                chunk_fcTL.delay_num = chunk_fcTL.delay_num;
                chunk_fcTL.delay_den = chunk_fcTL.delay_den;
                chunk_fcTL.dispose_op = LV_PNG_DISPOSE_OP_BACKGROUND;
                chunk_fcTL.blend_op = LV_PNG_BLEND_OP_SOURCE;
                ...
                *((uint32_t *)(fcTL + 34)) = lv_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
                ...
                for (int d = 0; d < frame->chunk_num; d++) {
                    lv_png_chunk_info *dchunk = frame->chunks + d;
                    if (dchunk->fourcc == LV_FOUR_CC('I', 'D', 'A', 'T')) {
                        uint32_t length = lv_swap_endian_uint32(dchunk->length + 4);
                        [result appendBytes:&length length:4]; //length
                        uint32_t fourcc = LV_FOUR_CC('f', 'd', 'A', 'T');
                        [result appendBytes:&fourcc length:4]; //fourcc
                        uint32_t sq = lv_swap_endian_uint32(apngSequenceIndex);
                        ...
                        uint32_t crc = lv_swap_endian_uint32((uint32_t)crc32(0, bytes, dchunk->length + 8));
                        [result appendBytes:&crc length:4]; //crc
                        
                        apngSequenceIndex++;
                    }
                }
                lv_png_info_release(frame);
            }
        }
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = LVCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, LivelyPresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == LivelyTypePNG) return [self _encodeAPNG];
    if (_type == LivelyTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(LivelyType)type quality:(CGFloat)quality {
    LivelyEncoder *encoder = [[LivelyEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(LivelyDecoder *)decoder type:(LivelyType)type quality:(CGFloat)quality {
...
@implementation UIImage (LivelyCoder)

- (instancetype)lv_imageByDecoded {
    if (self.lv_isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = LVCGImageCreateDecodedCopy(imageRef, YES);
    ...
    newImage.lv_isDecodedForDisplay = YES;
    return newImage;
}

- (BOOL)lv_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[LivelySpriteSheetImage class]]) return YES;
    NSNumber *num = objc_getAssociatedObject(self, @selector(lv_isDecodedForDisplay));
    return [num boolValue];
}

- (void)setLv_isDecodedForDisplay:(BOOL)isDecodedForDisplay {
    objc_setAssociatedObject(self, @selector(lv_isDecodedForDisplay), @(isDecodedForDisplay), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSData *)lv_imageDataRepresentation {
    return [self _lv_dataRepresentationForSystem:NO];
}

/// @param forSystem YES: used for system album (PNG/JPEG/GIF), NO: used for Lively (PNG/JPEG/GIF/WebP)
- (NSData *)_lv_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[Lively class]]) {
        Lively *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == LivelyTypeGIF ||
                    image.animatedImageType == LivelyTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = LVCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
