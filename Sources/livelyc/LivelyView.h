...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `LVAnimatedImage` protocol,
...
/**
 The LVAnimatedImage protocol declares the required methods for animated image
 display with LVAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to LVAnimatedImageView.image or LVAnimatedImageView.highlightedImage
 to display animation.
 
 See `Lively` and `LVFrameImage` for example.
 */
@protocol LVAnimatedImage <NSObject>
@required
