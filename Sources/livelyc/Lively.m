...
+ (Lively *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (Lively *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (Lively *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        LivelyDecoder *decoder = [LivelyDecoder decoderWithData:data scale:scale];
        LivelyFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
        ...
        self.lv_isDecodedForDisplay = YES;
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"LivelyScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"LivelyData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"LivelyScale"];
        [aCoder encodeObject:_decoder.data forKey:@"LivelyData"];
    } else {
...
#pragma mark - protocol LVAnimatedImage
