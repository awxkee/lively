...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, LivelyType) {
    LivelyTypeUnknown = 0, ///< unknown
    LivelyTypeJPEG,        ///< jpeg, jpg
    LivelyTypeJPEG2000,    ///< jp2
    LivelyTypeTIFF,        ///< tiff, tif
    LivelyTypeBMP,         ///< bmp
    LivelyTypeICO,         ///< ico
    LivelyTypeICNS,        ///< icns
    LivelyTypeGIF,         ///< gif
    ...
    LivelyTypeWebP,        ///< webp
    LivelyTypeAVIF,
    LivelyTypeOther,       ///< other image format
};
 ...
 */
typedef NS_ENUM(NSUInteger, LivelyDisposeMethod) {
 ...
 */
typedef NS_ENUM(NSUInteger, LivelyBlendOperation) {
 ...
 */
@interface LivelyFrame : NSObject <NSCopying>
...
@property (nonatomic) LivelyDisposeMethod dispose;     ///< Frame dispose method.
@property (nonatomic) LivelyBlendOperation blend;      ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    LivelyDecoder *decoder = [LivelyDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    LivelyDecoder *decoder = [[LivelyDecoder alloc] initWithScale:2.0];
 ...
 */
@interface LivelyDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) LivelyType type;          ///< Image data type.
 ...
 */
- (nullable LivelyFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in LivelyType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    LivelyEncoder *jpegEncoder = [[LivelyEncoder alloc] initWithType:LivelyTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    LivelyEncoder *gifEncoder = [[LivelyEncoder alloc] initWithType:LivelyTypeGIF];
 ...
 */
@interface LivelyEncoder : NSObject

@property (nonatomic, readonly) LivelyType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(LivelyType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(LivelyType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(LivelyDecoder *)decoder type:(LivelyType)type quality:(CGFloat)quality;
 ...
 */
@property (nonatomic) BOOL lv_isDecodedForDisplay;
 ...
 */
- (nullable NSData *)lv_imageDataRepresentation;
...
CG_EXTERN LivelyType LivelyDetectType(CFDataRef data);

/// Convert LivelyType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable LivelyTypeToUTType(LivelyType type);

/// Convert UTI (such as kUTTypeJPEG) to LivelyType.
CG_EXTERN LivelyType LivelyTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable LivelyTypeGetExtension(LivelyType type);



/// Returns the shared DeviceRGB color space.
CG_EXTERN CGColorSpaceRef LVCGColorSpaceGetDeviceRGB();

/// Returns the shared DeviceGray color space.
CG_EXTERN CGColorSpaceRef LVCGColorSpaceGetDeviceGray();

/// Returns whether a color space is DeviceRGB.
CG_EXTERN BOOL LVCGColorSpaceIsDeviceRGB(CGColorSpaceRef space);

/// Returns whether a color space is DeviceGray.
CG_EXTERN BOOL LVCGColorSpaceIsDeviceGray(CGColorSpaceRef space);



/// Convert EXIF orientation value to UIImageOrientation.
CG_EXTERN UIImageOrientation LVUIImageOrientationFromEXIFValue(NSInteger value);

/// Convert UIImageOrientation to EXIF orientation value.
CG_EXTERN NSInteger LVUIImageOrientationToEXIFValue(UIImageOrientation orientation);
 ...
 */
CG_EXTERN CGImageRef _Nullable LVCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
 ...
 */
CG_EXTERN CGImageRef _Nullable LVCGImageCreateCopyWithOrientation(CGImageRef imageRef,
...
/**
 ...
 @return A new image, or NULL if an error occurs.
 */
CG_EXTERN CGImageRef _Nullable LVCGImageCreateAffineTransformCopy(CGImageRef imageRef,
 ...
 */
CG_EXTERN CFDataRef _Nullable LVCGImageCreateEncodedData(CGImageRef imageRef, LivelyType type, CGFloat quality);


/**
 Whether WebP is available in Lively.
 */
CG_EXTERN BOOL LivelyWebPAvailable();
 ...
 */
CG_EXTERN NSUInteger LivelyGetWebPFrameCount(CFDataRef webpData);
 ...
 */
CG_EXTERN CGImageRef _Nullable LVCGImageCreateWithWebPData(CFDataRef webpData,
                                                           BOOL decodeForDisplay,
                                                           BOOL useThreads,
                                                           BOOL bypassFiltering,
                                                           BOOL noFancyUpsampling);

typedef NS_ENUM(NSUInteger, LivelyPreset) {
    LivelyPresetDefault = 0,  ///< default preset.
    LivelyPresetPicture,      ///< digital picture, like portrait, inner shot
    LivelyPresetPhoto,        ///< outdoor photograph, with natural lighting
    LivelyPresetDrawing,      ///< hand or line drawing, with high-contrast details
    LivelyPresetIcon,         ///< small-sized colorful images
    LivelyPresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is LivelyPresetDefault.
 @return WebP data, or nil if an error occurs.
 */
CG_EXTERN CFDataRef _Nullable LVCGImageCreateEncodedWebPData(CGImageRef imageRef,
                                                             BOOL lossless,
                                                             CGFloat quality,
                                                             int compressLevel,
                                                             LivelyPreset preset);
