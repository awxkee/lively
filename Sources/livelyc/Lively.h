...
//  Lively
//
//  Created by Radzivon Bartoshyk on 14/10/20.
...
#import "LivelyFrameImage.h"
#import "LivelySpriteSheetImage.h"
...
/**
 A Lively object is a high-level way to display animated image data.
 ...
 animation, try replace UIImageView with `LVAnimatedImageView`.
 
 Sample Code:
 
     // animation@3x.webp
     Lively *image = [Lively imageNamed:@"animation.webp"];
     LVAnimatedImageView *imageView = [LVAnimatedImageView alloc] initWithImage:image];
     [view addSubView:imageView];
    
 */
@interface Lively : UIImage <LVAnimatedImage>

+ (nullable Lively *)imageNamed:(NSString *)name; // no cache!
+ (nullable Lively *)imageWithContentsOfFile:(NSString *)path;
+ (nullable Lively *)imageWithData:(NSData *)data;
+ (nullable Lively *)imageWithData:(NSData *)data scale:(CGFloat)scale;

/**
 If the image is created from data or file, then the value indicates the data type.
 */
@property (nonatomic, readonly) LivelyType animatedImageType;
